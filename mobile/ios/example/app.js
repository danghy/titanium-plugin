// This is a test harness for the MobileAppTracker module.
// It shows sample calls to various methods supported by MobileAppTrakcer.


// open a single window
var win = Ti.UI.createWindow({
    backgroundColor:'white'
});

var label = Ti.UI.createLabel({text: "MAT Titanium Test App", top: 20, width: 230, height: 30 });
label.textAlign = Titanium.UI.TEXT_ALIGNMENT_CENTER;
win.add(label);

var mat = require('ti.mobileapptracker');
Ti.API.info("module is => " + mat);

var MAT_ADVERTISER_ID = "your_mat_advertiser_id";
var MAT_CONVERSION_KEY = "your_mat_conversion_key";
//var MAT_PACKAGE_NAME = "your_mat_app_package_name";

var button = Titanium.UI.createButton({ title: 'Start MAT', top: 60, width: 230, height: 30 });
button.addEventListener('click',function(e) {
    mat.initTracker([MAT_ADVERTISER_ID, MAT_CONVERSION_KEY]);
    
    mat.setAppleAdvertisingIdentifier("12345678-1234-1234-1234-123456789012", true);
    
    // Optional: Only required if the app's bundle id does not match the app package name set on MAT.  
    //mat.setPackageName(MAT_PACKAGE_NAME);
    
    // only for debugging
    //mat.setDelegate(true);
});
win.add(button);

var isMATDebugEnabled = false;
var button = Titanium.UI.createButton({ title: 'Toggle Debug Mode', top: 100, width: 230, height: 30 });
isMATDebugEnabled = !isMATDebugEnabled;
button.addEventListener('click',function(e) { mat.setDebugMode(isMATDebugEnabled); });
win.add(button);

var isMATDuplicatesAllowed = false;
var button = Titanium.UI.createButton({ title: 'Toggle Allow Duplicates', top: 140, width: 230, height: 30 });
isMATDuplicatesAllowed = !isMATDuplicatesAllowed;
button.addEventListener('click',function(e) { mat.setAllowDuplicates(isMATDuplicatesAllowed); });
win.add(button);

var button = Titanium.UI.createButton({ title: 'Measure Session', top: 220, width: 230, height: 30 });
button.addEventListener('click',function(e) { mat.measureSession(); });
win.add(button);

var button = Titanium.UI.createButton({ title: 'Measure Action', top: 300, width: 230, height: 30 });
button.addEventListener('click',function(e) { mat.measureAction(["purchase",
                                                    "ref1",
                                                    0.99,
                                                    "USD"]); });
win.add(button);

var button = Titanium.UI.createButton({ title: 'Measure Action With EventItems', top: 340, width: 230, height: 30 });
button.addEventListener('click',function(e) { 
    
    var eventItems = new Array();

                var eventItem1 = {"item":"item1",
                    "quantity":1,
                    "unit_price":0.99,
                    "revenue":0.99,
                    "attribute_sub1":"1",
                    "attribute_sub2":"2",
                    "attribute_sub3":"3",
                    "attribute_sub4":"4",
                    "attribute_sub5":"5"
                };
                eventItems[0] = eventItem1;
                
                var eventItem2 = {"item":"item2",
                    "quantity":2,
                    "unit_price":0.50,
                    "revenue":1
                };
                eventItems[1] = eventItem2;
    
    mat.measureAction(["purchase",
                                eventItems,
                                "ref2",
                                0.99,
                                "USD"]);
});

win.add(button);

var button = Titanium.UI.createButton({ title: 'Measure Action With IAP Receipt', top: 380, width: 230, height: 30 });
button.addEventListener('click',function(e) { 
    
    var eventItems = new Array();
                
                var eventItem1 = {"item":"item1",
                    "quantity":1,
                    "unit_price":0.99,
                    "revenue":0.99,
                    "attribute_sub1":"1",
                    "attribute_sub2":"2",
                    "attribute_sub3":"3",
                    "attribute_sub4":"4",
                    "attribute_sub5":"5"
                };
                eventItems[0] = eventItem1;
                
                var eventItem2 = {"item":"item2",
                    "quantity":2,
                    "unit_price":0.50,
                    "revenue":1
                };
                eventItems[1] = eventItem2;
    
    mat.measureAction(["purchase",
                                 eventItems,
                                 "ref3",
                                 0.99,
                                 "USD",
                                 0,
                                 getSampleiTunesIAPReceipt(),
                                 null]); 

});
win.add(button);


var button = Titanium.UI.createButton({ title: 'Test Setter Methods', top: 420, width: 230, height: 30 });
button.addEventListener('click',function(e) { 
    
    mat.setAge(23);
    mat.setAppAdTracking(true);
    mat.setAppleAdvertisingIdentifier("12345678-1234-1234-1234-123456789012", true);
    mat.setAppleVendorIdentifier("12345678-1234-1234-1234-123456789012");
    mat.setCurrencyCode("tempCurrencyCode");
    mat.setEventAttribute1("attr1");
    mat.setEventAttribute2("attr2");
    mat.setEventAttribute3("attr3");
    mat.setEventAttribute4("attr4");
    mat.setEventAttribute5("attr5");
    mat.setExistingUser(false);
    mat.setFacebookUserId("tempFacebookId");
    mat.setGender(1);
    mat.setGoogleAdvertisingId("12345678-1234-1234-1234-123456789012", false);
    mat.setGoogleUserId("tempGoogleId");
    mat.setJailbroken(false);
    mat.setLocation(1.1,2.2);
    mat.setLocationWithAltitude(3.3,4.4,5.5);
    mat.setPackageName("testPackageName");
    mat.setPayingUser(true);
    mat.setTRUSTeId("tempTrusteTPID");
    mat.setTwitterUserId("tempTwitterId");
    mat.setUseCookieTracking(false);
    mat.setUserEmail("tempUserEmail@tempUserCompany.com");
    mat.setUserId("tempUserId");
    mat.setUserName("tempUserName");
});
win.add(button);

var button = Titanium.UI.createButton({ title: 'Test Getter Methods', top: 460, width: 230, height: 30 });
button.addEventListener('click',function(e) { 
    
    var matId = mat.getMatId();
    var openLogId = mat.getOpenLogId();
    var paying = mat.getIsPayingUser();
    
    console.log('matId = ' + matId);
    console.log('openLogId = ' + openLogId);
    console.log('isPayingUser = ' + paying);
});
win.add(button);

win.open();

function getSampleiTunesIAPReceipt()
{
    return "{\"signature\" = \"\";\"purchase-info\" = \"\";\"environment\" = \"Sandbox\";\"pod\" = \"100\";\"signing-status\" = \"0\";}";
}

if (Ti.Platform.name == "android") {
    var proxy = MobileAppTrackerModule.createExample({
        message: "Creating an example Proxy",
        backgroundColor: "red",
        width: 100,
        height: 100,
        top: 100,
        left: 150
    });

    proxy.printMessage("Hello world!");
    proxy.message = "Hi world!.  It's me again.";
    proxy.printMessage("Hello world!");
    win.add(proxy);
}

